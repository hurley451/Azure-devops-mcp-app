...
| Planning reimplements ADO work-item create (patch doc + parent link) vs `work-items.ts`     | **Partially mitigated (R7.3)** — within the planning module, create + update now share `field-ops.ts` (one set of JSON-patch field builders), so they cannot diverge. Unifying with the _upstream_ `src/tools/work-items.ts` is still deferred (would couple to Microsoft tool code; higher risk, low payoff). |
